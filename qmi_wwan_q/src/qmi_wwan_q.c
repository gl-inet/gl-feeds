...
enum qmi_wwan_flags {
	QMI_WWAN_FLAG_QMAP = 1 << 16,
	QMI_WWAN_FLAG_OFF_LINK = 1 << 17,
	QMI_WWAN_FLAG_USE_RMNET = 1 << 18,	
};
...
module_param_named( one_card_mode, one_card_mode, uint, S_IRUGO );

//#define QMI_AGG_COUNT_FORCE_ONE
#ifdef QMI_AGG_COUNT_FORCE_ONE
static uint __read_mostly agg_count_force_one = 0;
module_param_named( agg_count_force_one, agg_count_force_one, uint, S_IRUGO );
#endif

//#define WORKAROUND_FOR_USB_TX_ZLP 512
#ifdef WORKAROUND_FOR_USB_TX_ZLP
static uint workaround_hint = 0;
module_param( workaround_hint, uint, S_IRUGO);
#endif
...
#endif
#ifdef WORKAROUND_FOR_USB_TX_ZLP
		if ((skb->len%WORKAROUND_FOR_USB_TX_ZLP) == 0) {
			if (skb_tailroom(skb)) {
				__skb_put(skb, 1);
			} else {
				__skb_push(skb, 1);
...
#endif
#ifdef WORKAROUND_FOR_USB_TX_ZLP
		if ((agg_skb->len%WORKAROUND_FOR_USB_TX_ZLP) == 0) {
			if (skb_tailroom(agg_skb)) {
				__skb_put(agg_skb, 1);
			} else {
				__skb_push(agg_skb, 1);
			}
			workaround_hint++;
		}
#endif
					...
					struct net_device *pNet)
{
	...
	if (priv->qmap_version == QMAPV1) {
		add_qhdr(skb, priv->mux_id, qmap_hdr, ip_offset);
		hdr_len = 4;
	}
	else if (priv->qmap_version == QMAPV5) {
...
#ifdef QMI_AGG_COUNT_FORCE_ONE
	if (pQmapDev->agg_count_force_one)
		return 0;
#endif
	...
	case 0x89F4: //SIOCDEVPRIVATE
		...
		if (pQmapDev->use_rmnet_usb && brmac_settings.id < qmap_mode) {
			...
			pr_info("ioctl 0x89F4 change qmapnet bridge(%d) lan mac -> %02x:%02x:%02x:%02x:%02x:%02x\n",
				brmac_settings.id,
				priv->bridge_self_mac[0], priv->bridge_self_mac[1], priv->bridge_self_mac[2],
				priv->bridge_self_mac[3], priv->bridge_self_mac[4], priv->bridge_self_mac[5]);
		}
			...
			pr_info("ioctl 0x89F4 change usbnet bridge(%d) lan mac -> %02x:%02x:%02x:%02x:%02x:%02x\n",
				...
				pQmapDev->bridge_self_mac[1], pQmapDev->bridge_self_mac[2], pQmapDev->bridge_self_mac[3],
				pQmapDev->bridge_self_mac[4], pQmapDev->bridge_self_mac[5]);
		}
		else {
...
static struct sk_buff *qmap_qmi_wwan_tx_fixup(struct usbnet *dev, struct sk_buff *skb, gfp_t flags) {
	...
	if (unlikely(pQmapDev == NULL)) {
		...
		if (likely(skb)) {
			...
			if (skb) {
				if(pQmapDev->qmap_version == QMAPV1)
	...
	struct sk_buff *skb_in, struct sk_buff_head *skb_chain)
{
	...
	if (pQmapDev->qmap_version == QMAPV5)
...
static int qmap_qmi_wwan_rx_fixup(struct usbnet *dev, struct sk_buff *skb_in)
{
	...
	if (pQmapDev->agg_count_force_one) {
		if (pQmapDev->qmap_version == QMAPV1)
			skb_pull(skb_in, 4);
		else if (pQmapDev->qmap_version == QMAPV5)
			skb_pull(skb_in, 8);
		else
			return 0;
...
static int qmi_wwan_bind(struct usbnet *dev, struct usb_interface *intf)
{
	...
	if (ether_addr_equal(dev->net->dev_addr, zero_mac_addr)){
		eth_hw_addr_random(dev->net);
		printk(KERN_INFO "%s: %s:if get zero mac, random mac addr  %pM\n", 
	...
	if (!status)
	{
		...
		if (dev->driver_info->flags & FLAG_NOARP)
		{
			...
			int qmap_enabled = !!(dev->driver_info->data&QMI_WWAN_FLAG_QMAP);
			int link_off = !!(dev->driver_info->data&QMI_WWAN_FLAG_OFF_LINK);
			int use_rmnet = !!(dev->driver_info->data&QMI_WWAN_FLAG_USE_RMNET);

			pQmapDev->qmap_mode = qmap_mode;
			if (qmap_enabled && pQmapDev->qmap_mode == 0) {
			...
			if (pQmapDev->qmap_mode) {
				...
				pQmapDev->link_state = link_off ? 0 : 1;
				...
				if (pQmapDev->use_rmnet_usb == 1 && pQmapDev->qmap_mode == 1 && one_card_mode) {
					pQmapDev->use_rmnet_usb = 0;
				}

#ifdef QMI_AGG_COUNT_FORCE_ONE
				if (pQmapDev->qmap_mode == 1 && pQmapDev->use_rmnet_usb == 0 && agg_count_force_one {
					pQmapDev->agg_count_force_one = 1;
					pQmapDev->qmap_size = 1524; //mtu + ether_hdr + qmap_hdr
					dev->rx_urb_size = pQmapDev->qmap_size;
				}
#endif
				...
				pQmapDev->rmnet_info.iface_id = intf->cur_altsetting->desc.bInterfaceNumber;
...
static const struct driver_info qmi_wwan_raw_ip_info_mdm9x07 = {
	...
	.data = (QMAPV1<<8)|4, //QMAPV1 and 4KB
};

// mdm9x40/sdx12/sdx20/sdx24 share the same config
static const struct driver_info qmi_wwan_raw_ip_info_mdm9x40 = {
	qmi_wwan_raw_ip_info
	//.data =  QMI_WWAN_FLAG_QMAP | QMI_WWAN_FLAG_OFF_LINK | (QMAPV1<<8)|16, //QMAPV1 and 16KB
	.data =  QMI_WWAN_FLAG_USE_RMNET | QMI_WWAN_FLAG_QMAP | QMI_WWAN_FLAG_OFF_LINK | (QMAPV1<<8)|16, //QMAPV1 and 16KB
};

static const struct driver_info qmi_wwan_raw_ip_info_sdx55 = {
	qmi_wwan_raw_ip_info
	.data = QMI_WWAN_FLAG_USE_RMNET | QMI_WWAN_FLAG_QMAP | QMI_WWAN_FLAG_OFF_LINK | (QMAPV5<<8)|31, //QMAPV5 and 31KB
};
