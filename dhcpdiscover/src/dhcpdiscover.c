...
int get_dhcp_offer(int sock)
{
    ...
    for (responses = 0, valid_responses = 0;!valid_responses;) {
        time(&current_time);
...
int add_dhcp_offer(struct in_addr source, dhcp_packet* offer_packet)
{
    ...
    for (x = 4; x < MAX_DHCP_OPTIONS_LENGTH;) {
        ...
        else if (option_type == DHCP_OPTION_NETMASK) {
            memcpy(&netmask, &offer_packet->options[x], 4);
        }
        else if (option_type == DHCP_OPTION_ROUTER) {
            memcpy(&router, &offer_packet->options[x], 4);
        }
        else if (option_type == DHCP_OPTION_DNS) {
            memcpy(&dns[0], &offer_packet->options[x], 4);
            if(option_length == 8)
                memcpy(&dns[1], &offer_packet->options[x+4], 4);
        }
    ...
    memcpy(&new_offer->netmask,&netmask, 4);
    ...
    if (prometheus) {
        ...
        fprintf(stdout, "netmask: %s\n", inet_ntoa(new_offer->netmask)); 
        fprintf(stdout, "router: %s\n", inet_ntoa(router)); 
        fprintf(stdout, "dns1: %s\n", inet_ntoa(dns[0])); 
        fprintf(stdout, "dns2: %s\n", inet_ntoa(dns[1]));
        fprintf(stdout, "LeaseTime: %lu\n", (unsigned long)new_offer->lease_time); 
